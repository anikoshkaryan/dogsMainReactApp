...
export default class App extends Component {
    constructor(props) {
        ...
        this.state = {
            questions_answers: [
                {
                    id: 1,
                    title: "Выберите размер собаки",
                    key: 'dog_size',
                    initialRadioPos: -1,
                    answers: [
                        {label: 'Очень маленькие (до 5кг и 30см)', value: 'very_small'},
                        {label: 'Маленькие (до 5-10кг и 30-40см)', value: 'small'},
                        {label: 'Средние (до 10-20кг и 40-56см)', value: 'medium'},
                        {label: 'Большие (до 20-30кг и 56-65см)', value: 'big'},
                        {label: 'Очень большие (до 50кг и 60см)', value: 'very_big'},
                        {label: 'Не имеет значение!', value: "null"},
                    ],
                },
                {
                    id: 2,
                    initialRadioPos: -1,
                    key: 'dog_dominate',
                    title: "Склонность к доминированию",
                    answers: [
                ...
                },
                {
                    id: 3,
                    initialRadioPos: -1,
                    key: 'content_apartment',
                    title: "Содержание в квартире",
                    answers: [
                        {label: 'Да', value: true},
                ...
                },
                {
                    ...
                    title: "Опыт владельца",
                    answers: [
                        {label: 'Без опыта', value: 'no_experience'},
                        {label: 'С опытом', value: 'experience'},
                        {label: 'Не имеет значение!', value: "null"},
                    ],
                ...
                },
                {
                    ...
                    initialRadioPos: -1,
                    key: 'difficulty_care',
                    title: "Сложность в уходе",
                    answers: [
                        {label: 'Сложно', value: 'complicated'},
                        {label: 'Нормально', value: 'normally'},
                ...
                },
                {
                    ...
                    initialRadioPos: -1,
                    key: 'bark_and_howl',
                    title: "Склонность к лаю и завываниям",
                    answers: [
                        {label: 'Высокая', value: 'high'},
                        {label: 'Низкая', value: 'low'},
                        {label: 'Не имеет значение!', value: "null"},
                ...
                },
                {
                    ...
                    initialRadioPos: -1,
                    key: 'shedding_level',
                    title: "Уровень линьки",
                    answers: [
                        {label: 'Повышенный', value: 'high'},
                        {label: 'Нормальный', value: 'medium'},
    ...
    handleClearRadioForm =  () => {
        this.setState({
            formKey: Math.random()
        })
    }

    handleSortDogs = async () => {
        let {values} = this.state
        let userToken = await AsyncStorage.getItem('userToken');
        let AuthStr = 'Bearer ' + userToken;
        try {
            if (Object.keys(this.state.values).length !== 13) {
                this.setState({
                    alertMessage: true
                })
                return false
            } else {
                fetch('http://80.78.246.59/dogs-app/public/api/question-dog-data', {
                    method: 'post',
                    headers: {
                    ...
                    body: JSON.stringify({
                        ...
                        dog_dominate: values.dog_dominate,
                        content_apartment: values.content_apartment,
                        child_compatibility: values.child_compatibility,
                        activity: values.activity,
                        owner_experience: values.owner_experience,
                        difficulty_care: values.difficulty_care,
                        guard_qualities: values.guard_qualities,
                        learnability: values.learnability,
                        independence: values.independence,
                        friendliness: values.friendliness,
                        bark_and_howl: values.bark_and_howl,
                        shedding_level: values.shedding_level,
                    })
                }).then((res) => {
                    return res.json()
                }).then((res) => {
                    if (res.success === true) {
                        this.handleClearRadioForm()

                        this.setState({
                            values: {}
                        })
                        this.props.navigation.navigate("QuestionsDogs", {
                            dog_info: res.data
                        })

                    }
                    console.log(this.state.initialRadioPos, 'INIT')
                })
            }
        } catch (e) {
            console.log(e)
        ...
        return (
            <SafeAreaView style={styles.container}>
                ...
                <LinearGradient colors={['#E1C1B7', '#FFFFFF']} style={styles.dogs_breeds_wrapper}>
                    ...
                    >
                        <TouchableOpacity onPress={this.toggleAlert}
                                          style={{alignSelf: 'flex-end', top: -55, marginRight: 10}}>
                            <Text style={{fontSize: 18,}}>x</Text>
                        </TouchableOpacity>
                        <Text style={{marginTop: -16, marginBottom: 55, fontSize: 15}}>Ответьте на все вопросы!</Text>
                    </FancyAlert>
                    <ScrollView style={styles.dogs_items_scrollView}>
                        {this.getQuestions().map((item) => (
                            <View style={styles.questions_answers_main_wrapper} key={item.id}>
                                ...
                                <View>
                                    <Text>
                                        <RadioForm
                                            ...
                                            onPress={(value) => {
                                                setTimeout(() => {
                                                    this.setState({values: Object.assign(this.state.values, {[item.key]: value})})
                                                }, 50)
                                            }}
                                        />
                                    </Text>
                    ...
                    <LinearGradient colors={['#c9a477', '#b56422']} style={styles.complete_button_parent}>
                        <TouchableOpacity style={styles.complete_btn} onPress={this.handleSortDogs}>
