...
export default class App extends Component {
    ...
    componentDidMount() {
        this.handleGetDogFoodData()
        this.handleGetDogsId()
    }


    handleGetDogsId = async () => {
        ...
        try {
            fetch(`http://80.78.246.59/dogs-app/public/api/dogs-data`, {
                ...
                let newDat = response.data.map(function (obj) {
                    return {
                        label: obj.parod_name,
                        value: obj.id
                    }
                });
    ...
    handleGetDogFoodData = async () => {
        let userToken = await AsyncStorage.getItem('userToken');
        let AuthStr = 'Bearer ' + userToken;

        let api = `http://80.78.246.59/dogs-app/public/api/feeds-data`

        if (this.state.dog_id !== null){
            api = `http://80.78.246.59/dogs-app/public/api/feeds-data?dog_id=${this.state.dog_id}`
        }
        try {
            fetch(api, {
                method: 'post',
                ...
                if (res.data?.all_feeds) {
                    this.setState({
                        foods_list: res.data?.all_feeds
                    })
                } else {
                    this.setState({
                        foods_list: res.data.dog_feed_data
                    })
                }

                //   console.log(this.state.foods_list, 'FOODS')
            })
        } catch (e) {
            console.log(e)
        }
            ...
            <SafeAreaView style={styles.container}>
                ...
                <LinearGradient colors={['#E1C1B7', '#FFFFFF']} style={styles.dogs_breeds_wrapper}>
                    ...
                    <ScrollView style={styles.dogs_items_scrollView}>
                        ...
                        {this.state.foods_list.map((dogsFood) => {
                            return (
                                <TouchableOpacity style={styles.dog_img_info_main_wrapper} key={dogsFood.id}
                                                  onPress={() => this.redirectToDogsFoodSinglePage(dogsFood)}>
                                    <View style={styles.dog_img_box}>
                                        <View style={styles.dog_img}>
                                            <Image style={styles.dog_img_child}
                                                   source={{uri: this.state.imageUrl + dogsFood.feed_photo}}/>
                                        </View>
                                    ...
                                    <View style={styles.dog_info_main_wrapper}>
                                        ...
                                        <Text style={styles.dog_info}> {dogsFood.description} </Text>
